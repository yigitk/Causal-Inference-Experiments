...
def gsl_sf_pow_int_e(x, n, result):
    ...
    if n < 0:
        ...
        if x == 0.0:
            u = 1.0 / x
            result_val = u if n % 2 == 1 else u * u
            result_err = inf
            result.val = result_val
            result.err = result_err
            print("overflow err")
    ...
    while True:
        ...
        if n == 0:
            break
    result_val = value
    result_err = 2.0 * GSL_DBL_EPSILON * (count + 1.0) * fabs(value)
    result.val = result_val
    result.err = result_err
    return GSL_SUCCESS
...
def gsl_sf_pow_int(x, n):
    ...
    return_val = EVAL_RESULT(gsl_sf_pow_int_e(x, n, result), result)
    return return_val
...
def gsl_sf_synchrotron_1_e(x, result):
    if x < 0.0:
        ...
        result_val = 2.14952824153447863671 * z * cf
        result.val = result_val
        ...
        # add
        result_c1_val_IV = result_c1.val
        result_c2_val_IV = result_c2.val
        #result.val  = px * result_c1.val - px11 * result_c2.val - c0 * x
        result_val = px * result_c1_val_IV - px11 * result_c2_val_IV - c0 * x
        result.val = result_val

        result_c1_err_IV = result_c1.err
        result_c2_err_IV = result_c2.err
        result_err = px * result_c1_err_IV + px11 * \
            result_c2_err_IV + c0 * x * GSL_DBL_EPSILON
        result.err = result_err
        ...
        result_c1_val_IV = result_c1.val
        result_val = sqrt(x) * result_c1_val_IV * exp(c0 - x)
        result.val = result_val

        result_val_IV = result.val
        result_err = result.err
        result_err = 2.0 * GSL_DBL_EPSILON * result_val_IV * (fabs(c0-x)+1.0)
        result.err = result_err
        return GSL_SUCCESS
    else:
...
good_dict = {}
args0 = np.arange(0.1, 11, 0.01)
cnt = 0
for arg0 in args0:
    good_dict[cnt] = gsl_sf_synchrotron_1(arg0)
    cnt += 1
