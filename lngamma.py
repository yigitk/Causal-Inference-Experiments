...
def psi_n_xg0(n, x, result):
    if n == 0:
        ...
        stat_e = gsl_sf_exp_mult_err_e(ln_nf.val, ln_nf.err,
			hzeta.val, hzeta.err,
			result)
        if GSL_IS_EVEN(n):
...
def lngamma_sgn_0(eps, lgn, sgn):
    ...
    lgn.val = log(gee / fabs(eps))
    lgn.err = 4.0 * GSL_DBL_EPSILON * fabs(lgn.val)
    sgn = GSL_SIGN(eps)
...
def gsl_sf_lngamma_e(x, result):
    if fabs(x - 1.0) < 0.01:
        ...
        if s == 0.0:
            ...
            result.val = M_LNPI - (log(_as) + lg_z.val)
...
for i in np.arange(0.01, 4.0, 0.01):
    print(gsl_sf_lngamma(i))
