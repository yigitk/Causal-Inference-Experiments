...
def gsl_sf_bessel_cos_pi4_e(y, eps, result):
    ...
    result_val = (ceps * s - seps * d) / sqrt(2)
    result.val = result_val
    result_err = 2.0 * GSL_DBL_EPSILON * (fabs(ceps) + fabs(seps)) * abs_sum / sqrt(2)
    result.err = result_err
    if y > 1.0 / GSL_DBL_EPSILON:
        ...
        result_err = result.err
        result_err *= 256.0 * y * GSL_SQRT_DBL_EPSILON
        result.err = result_err
    return GSL_SUCCESS
...
def gsl_sf_bessel_J0_e(x, result):
    ...
    if y < 2.0 * GSL_SQRT_DBL_EPSILON:
        ...
        ct_val_IV = ct.val
        stat_cp = gsl_sf_bessel_cos_pi4_e(y, ct_val_IV / y,  cp)
        sqrty = sqrt(y)
        ca_val_IV = ca.val
        ampl = (0.75 + ca_val_IV) / sqrty

        cp_val_IV = cp.val
        result_val = ampl * cp_val_IV
        result.val = result_val

        cp_val_IV = cp.val
        ca_err_IV = ca.err
        cp_err_IV = cp.err
        result_err = fabs(cp_val_IV) * ca_err_IV / sqrty + fabs(ampl) * cp_err_IV
        result.err = result_err

        result_val_IV = result.val
        result_err = result.err
        result_err += GSL_DBL_EPSILON * fabs(result_val)
        result.err = result_err
...
def EVAL_RESULT(fn, result):
    ...
    return_val_IV = result.val
    return return_val_IV
...
#good_dict is imported by the localizer
good_dict = {}
